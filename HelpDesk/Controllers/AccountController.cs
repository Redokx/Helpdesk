    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "Nie prawidłowy login lub hasło. Zalogowanie nie powiodło się.");
                    return View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new Uzytkownik { UserName = model.UserName, Email = model.Email, Imie = model.Imie, Nazwisko = model.Nazwisko, EmailConfirmed = true };
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(user.Id, "Wiadomość z aplikacji Helpdesk. Potwierdź swoje konto",
                        "Potwierdź sowje konto klikając <a href=\"" + callbackUrl + "\">TUTAJ</a>");
                    TempData["Potwierdzenie"] = "Użytkownik: " + model.UserName + " został dodany.";
                    return RedirectToAction("Index", "Uzytkowniks");
                }
                AddErrors(result);
            }
            return View(model);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "Helpdesk - zmiana hasła", "W celu zmiany hasła, prosze nacisnąć na poniższy link: </br> <a href=\"" + callbackUrl + "\">ZMIANA HASŁA</a>");
        ...
        //    var db = HttpContext.GetOwinContext().Get<HelpdeskContext>();

        //    // find the user. I am skipping validations and other checks.
        //    var user = db.Users.Where(x => x.Id == userId).FirstOrDefault();

        //    return new FileContentResult(user., "image/jpeg");
        //}
